...
public class Pump implements ModbusDevice<Integer> {
    ...
    private Register<Integer> generateCurrentConsumption(Integer number) {
    ...
    private Register<Integer> generateOverPressure(Integer number) {
        if (this.registers.get(STATE.ordinal()).getValue() == OFF.ordinal()) return new Register<>(number, OFF.ordinal());
